 ...
 * things Finance itself needs to act on: new pending payment uploads and new scholarship
 * applications. Same per-item read-state mechanism as getStudentNotifications
 * (NotificationRead, keyed by this item's id), so it slots into DashboardShell's existing
 * notifications prop as-is.
 */
export async function getFinanceNotifications(userId: string): Promise<NotificationItem[]> {
  const [decidedLeaveRequests, pendingPayments, pendingScholarships] = await Promise.all([
