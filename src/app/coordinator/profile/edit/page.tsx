...
export default async function CoordinatorEditProfilePage() {
  const coordinator = await requireRole(["PROGRAM_COORDINATOR"]);
  const user = await prisma.user.findUniqueOrThrow({ where: { id: coordinator.id } });
